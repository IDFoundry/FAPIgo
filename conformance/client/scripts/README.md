...
go run ./cmd/conformance-client -profile=federation
```
...
- `-profile` selects which plan to run — `baseline`
  ...
  objects and JARM), `ciba` (`fapi-ciba-id1-client-test-plan` — see its
  own section below; unlike the other two, most of its modules are
...
Drives `openid-federation-entity-joined-to-test-federation-rp-test-plan`
(`federation.go`, not `main.go`'s own `run`/`buildModuleClient` — this
plan needs its own live-hosted Entity Configuration endpoint and
federation-based discovery, both different enough from every other
profile to need their own driving functions, `runFederationRP`/
`runFederationModule`/`buildFederationModuleClient`). This is the
architectural mirror of everything else this driver does: here, the
suite plays the OP (self-hosting its own Trust Anchor, exactly like
`cmd/conformance-as`'s `-federation-trust-anchor-admin` work drives on
the AS side — same alias-based predictable-Entity-Identifier trick, for
the same race-avoidance reason), and this driver is the one that gets
resolved through federation, rather than resolving a client through it.

Every key both sides sign with is generated by this driver itself, never
the suite — its own federation signing key and client-authentication
key, plus three more (`federation.op_ec_jwks`, `federation.op_server_jwks`,
`federation_trust_anchor.trust_anchor_jwks`) the suite reads straight out
of the plan config and signs *its own* mock OP/Trust Anchor identities
with (confirmed against `OpenIDFederationClientTest.java`'s own
`configure()`, which never generates any key material of its own). Since
this driver already holds the Trust Anchor's own public half locally the
moment it generates that key, `DiscoverViaFederation`'s own trust anchor
is pinned from a value made up in-process, not fetched or configured
externally.

Unlike every other profile, this one needs a real inbound listener:
`federation_server.go` starts an HTTPS server (a throwaway self-signed
cert, SAN covering `host.docker.internal` — Docker Desktop's own DNS
name for reaching the host machine from inside a container) serving this
driver's own signed Entity Configuration at
`/.well-known/openid-federation`, reachable by the suite's own dockerized
backend. This was *not* obvious going in: the plan's own `server_metadata`
variant name (`static` vs `discovery`) suggests a static config value
might avoid needing this — it doesn't, for this specific plan.
`loadClientPublicJwksFromRPMetadata` (the suite's own condition,
`OpenIDFederationClientTest.java`) unconditionally fetches the RP's live
entity configuration to validate an incoming authorization/PAR request's
signature, with no static-config shortcut in that particular code path —
